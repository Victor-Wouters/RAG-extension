...
def evaluate_with_Q(model, embeddings, combined_texts, beta = 1):
    ...
    for query in data['queries']:
        ...
        query_vector =  embed.compute_bow_wcd([q],model)
        cosine_similarities = cosine_similarity(query_vector, embeddings)

        threshold = 0.55

        # Top-k retrieval with threshold filtering
        top_k_indices = [index for index in np.argsort(cosine_similarities[0])[::-1] if cosine_similarities[0][index] > threshold]
